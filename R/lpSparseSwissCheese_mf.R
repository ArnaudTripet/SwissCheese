...
lpSparseSwissCheese_mf <- function(X ,d = NULL, w = NULL, k_min = NULL, k_max = NULL, rand = TRUE, test = FALSE , subset = c(1:ncol(X)) )
{
  ##------------------
  ##  Initialization
  ##------------------
  ...
  Xm_mf <- as.matrix(X2[Sm,colSums(R)!=nm]) #--compute the distance only on variables containing missing values
  Xr_mf <- as.matrix(X2[Sr,colSums(R)!=nm])
  
  ##---Compute the Euclidean distance between respondents and non respondents
  if(is.null(w)){ w <- rep(1, sum(colSums(R)!=nm)) }else{ w <- w[colSums(R)!=nm] }
  if(is.null(k_max)){ k_max <- nr }
  D3   <- rep(0, k_max*nm)
  ind3 <- rep(0, k_max*nm)
  
  for(i in 1:nm){
    d_tmp3                          <- (sqrt(rowSums(t(t(Xr_mf)*w-Xm_mf[i,]*w)^2)))/ncol(Xr_mf)
