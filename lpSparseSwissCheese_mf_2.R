...
#' \code{\link[Rglpk:Rglpk_solve_LP]{Rglpk::Rglpk_solve_LP}}, \code{\link[slam:simple_triplet_matrix]{slam::simple_triplet_matrix}}, \code{\link[missForest:missForest]{missForest:missForest}}
#'
#' @examples
...
lpSparseSwissCheese_mf_2 <- function(X, d = NULL, w = NULL, k_min = NULL, k_max = NULL, rand = TRUE, test = FALSE , subset = c(1:ncol(X)) )
{
  ##------------------
  ##  Initialization
  ##------------------
  ...
  #--- Standardized data MF
  X_init <- X2
  var_0 <- which(!apply(X2, 2, function(x) sd(x[!is.na(x)]) == 0))
  X0_mf <- X2[,var_0]
  for(i in 1:length(var_0)){
    X0_mf[!is.na(X0_mf[,i]),i] <- as.vector(scale(X0_mf[!is.na(X0_mf[,i]),i]))
  }
  X2[,var_0] <- X0_mf
  ...
  Xm_mf <- as.matrix(X2[Sm,])
  Xr_mf <- as.matrix(X2[Sr,])
  ...
  # ind2 <- rep(0, k_max*nm)
  # for(i in 1:nm){
  #   d_tmp2                          <- (sqrt(rowSums(t(t(Xr_mf)*w-Xm_mf[i,]*w)^2)))/ncol(X)
  #   ind2[((i-1)*k_max+1):(i*k_max)] <- which(order(d_tmp2) %in% (1:k_max))
  #   D2[((i-1)*k_max+1):(i*k_max)]   <- d_tmp2[which(order(d_tmp2) %in% (1:k_max))]
  #   # D_all[,i]                      <- d_tmp
  # }
  ...
  ind3 <- rep(0, k_max*nm)
  for(i in 1:nm){
    d_tmp3                          <- (sqrt(rowSums(t(t(Xr_mf)*w-Xm_mf[i,]*w)^2)))/ncol(X)
    ind3[((i-1)*k_max+1):(i*k_max)] <- which(order(d_tmp3) %in% (1:k_max))
    D3[((i-1)*k_max+1):(i*k_max)]   <- d_tmp3[which(order(d_tmp3) %in% (1:k_max))]
    # D_all[,i]                      <- d_tmp
  }
  ...
  # 
  # for(i in 1:nm){
  #   d_tmp2                          <- (sqrt(rowSums(t(t(Xr_mf)*R[i,]*w-Xm_mf[i,]*R[i,]*w)^2)))/sum(R[i,])
  #   ind2[((i-1)*k_max+1):(i*k_max)] <- which(order(d_tmp2) %in% (1:k_max))
  #   D2[((i-1)*k_max+1):(i*k_max)]   <- d_tmp2[which(order(d_tmp2) %in% (1:k_max))]
  #   # D_all[,i]                      <- d_tmp
  # }
  ...
  for(j in 1:J){
    ...
    sum_max_j <- 0
    for(v in R1_j){
      ind_tmp   <- ind3[((v-1)*k_max+1):(v*k_max)]
  ...
  if(!is.null(k_min)){
    p      <- 1/k_min
    bounds <- list(upper = list(ind3 = 1:max(A_tmp[,2]), val = rep(p,max(A_tmp[,2]))))
    
    lp_res <- Rglpk::Rglpk_solve_LP(obj = D3,
                                    mat = A_sparse,
                                    dir = dir_sparse,
                                    rhs = B,
                                    max = FALSE,
  ...
  for(v in 1:nm){
    prob_tmp                                 <- rep(0,nr)
    prob_tmp[ind3[((v-1)*k_max+1):(v*k_max)]] <- lp_res[((v-1)*k_max+1):(v*k_max)]
